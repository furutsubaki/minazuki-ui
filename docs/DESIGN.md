...
| 200 | 0.88 | × 0.35 | 控えめな背景・ボーダー |
| 300 | 0.76 | × 0.65 | 抑えたテキスト、補助表示 |
| 400 | 0.65 | × 1.00 | 基本色（ボタン、アイコン、ステータス表示） |
| 500 | 0.55 | × 0.65 | ホバー・フォーカス時 |
| 600 | 0.45 | × 0.35 | プレス（押下）状態、強調テキスト |
...
**Yellow の lightnessOffset について**: sRGB ガモットの制約上、Yellow（Hue 95°）は L=0.65 付近では鮮やかな黄色にならず金色〜オリーブになる。Warning ロールは `lightnessOffset: 0.20` でラダー全体の明度を底上げすることでこれを回避する。
